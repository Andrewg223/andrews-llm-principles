...
| [Sanity-check claims](sanity-check-claims.md) | The LLM confidently says things that fall apart under simple arithmetic | The LLM catches its own impossible claims before you do. Promises survive the math against your actual constraints. | <ul><li>LLM promises "ten podcast episodes recorded in a six-hour workshop" without doing the math</li><li>LLM writes "supports 1M concurrent users" with no idea of the actual infrastructure</li><li>LLM claims a course is "accredited" when no accreditation has been applied for</li><li>LLM says "delivered in two weeks" without checking team capacity</li></ul> |
...
3. **Stack them.** Each principle handles a different failure mode. Using all five together compounds the effect — narrow framing covers *when to step back instead of keep going*, proactivity covers *how the work gets done*, the claim audit covers *what the LLM says is true*, Karpathy covers *how to do the work cleanly*, and the five-whys self-audit covers *which questions are even worth asking you*.
